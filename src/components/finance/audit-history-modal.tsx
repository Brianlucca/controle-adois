...
import { Check, Copy, History, Loader2, RotateCcw, Search, Trash2, X } from "lucide-react";
...
export function AuditHistoryModal({ onClose, onRestored }: { onClose: () => void; onRestored: () => void }) {
  const [logs, setLogs] = useState<AuditLog[]>([]); const [loading, setLoading] = useState(true); const [restoring, setRestoring] = useState(""); const [search, setSearch] = useState(""); const [copiedLogId, setCopiedLogId] = useState("");
  ...
  async function copyId(id: string, logId: string) { await navigator.clipboard.writeText(id); setCopiedLogId(logId); window.setTimeout(() => setCopiedLogId((current) => current === logId ? "" : current), 2000); }
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/80 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
    <div className="flex max-h-[94dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-2xl border border-white/10 bg-[#0f1420] shadow-2xl sm:max-h-[88vh] sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto p-3 sm:p-5">{loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-indigo-300" /></div> : !logs.length ? <div className="py-20 text-center text-slate-500">Nenhuma atividade registrada.</div> : !filteredLogs.length ? <div className="py-20 text-center text-slate-500">Nenhum resultado para esta pesquisa.</div> : <div className="space-y-3">{filteredLogs.map((log) => <article key={log.id} className="rounded-xl border border-white/10 bg-white/[0.025] p-4">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><div><div className="flex items-center gap-1.5"><p className="break-all font-mono text-[11px] text-slate-500">ID: {log.entityId}</p><button type="button" onClick={() => void copyId(log.entityId, log.id)} className="inline-flex h-7 shrink-0 items-center gap-1 rounded-md px-1.5 text-slate-500 transition-colors hover:bg-white/10 hover:text-slate-200" title="Copiar ID" aria-label="Copiar ID da transação">{copiedLogId === log.id ? <><Check size={13} className="text-emerald-400" /><span className="text-[10px] text-emerald-400">Copiado</span></> : <Copy size={13} />}</button></div><p className="mt-1 text-xs text-slate-400">Responsável: {log.actorName}</p></div>{log.action === "deleted" && latestByTransaction.get(log.entityId) === "deleted" && <Button onClick={() => restore(log.entityId)} disabled={restoring === log.entityId} className="h-9 bg-emerald-600 text-white hover:bg-emerald-500">{restoring === log.entityId ? <Loader2 className="mr-2 animate-spin" size={14} /> : <RotateCcw className="mr-2" size={14} />}Restaurar</Button>}</div><Changes before={log.before} after={log.after} />
      </article>)}</div>}</div>
    </div></div>;
}
